	...
	vorpal
	  ...
	  .action(function(args, callback) {
	    ...
	    var passphrase = require("bip39").generateMnemonic();
			...
			self.log("WIF     - private:",persona.crypto.getKeys(passphrase).toWIF());
			self.log("Address - public :",persona.crypto.getAddress(persona.crypto.getKeys(passphrase).publicKey));
			callback();
	  });
	var account=null;
	vorpal
